...
export class FriendsService {
  ...
  async sendRequest(userId: string, friendUsername: string) {
    if (!friendUsername)
      throw new BadRequestException('Нікнейм не може бути порожнім');
    ...
    if (friend.id === userId)
      throw new BadRequestException('Ви не можете додати себе в друзі');
    ...
    if (existing) {
      if (existing.status === 'pending')
        throw new ConflictException('Запит вже надіслано');
      if (existing.status === 'accepted')
        throw new ConflictException('Ви вже друзі');
      if (existing.status === 'blocked')
        throw new ConflictException('Користувач заблокований');
    }
      ...
      include: {
        friend: {
          select: {
            id: true,
            username: true,
            profile: { select: { avatarUrl: true, level: true } },
          },
        },
