...
export class TournamentController {
    ...
    create(
        @Request() req: any,
        @Body() body: { name: string; maxParticipants?: number; prizePool?: number; entryFee?: number; rules?: string },
    ) {
    ...
    join(@Request() req: any, @Param('id') id: string) {
        return this.tournamentService.joinTournament(req.user.id, req.user.username, id);
    }
