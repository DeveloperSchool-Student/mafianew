...
export class TradeService {
  ...
  async createTrade(
    ...
    if (
      !Number.isFinite(params.offerAmount) ||
      !Number.isFinite(params.requestAmount)
    ) {
    ...
    if (
      params.offerCurrency === 'SOFT' &&
      senderWallet.soft < params.offerAmount
    ) {
      throw new BadRequestException('Недостатньо софт-валюти.');
    }
    if (
      params.offerCurrency === 'HARD' &&
      senderWallet.hard < params.offerAmount
    ) {
  ...
  async acceptTrade(userId: string, tradeId: string) {
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      if (!senderWallet || !receiverWallet)
        throw new NotFoundException('Гаманці не знайдені.');

      // 1. Check sender's offer AGAIN to ensure they still have it
      if (
        trade.offerCurrency === 'SOFT' &&
        senderWallet.soft < trade.offerAmount
      ) {
        throw new BadRequestException('У відправника недостатньо коштів.');
      }
      if (
        trade.offerCurrency === 'HARD' &&
        senderWallet.hard < trade.offerAmount
      ) {
        throw new BadRequestException('У відправника недостатньо коштів.');
      }
      ...
      if (
        trade.requestCurrency === 'SOFT' &&
        receiverWallet.soft < trade.requestAmount
      ) {
        throw new BadRequestException('У вас недостатньо софт-валюти.');
      }
      if (
        trade.requestCurrency === 'HARD' &&
        receiverWallet.hard < trade.requestAmount
      ) {
  ...
  async rejectTrade(userId: string, tradeId: string) {
    ...
    const notifyId =
      trade.senderId === userId ? trade.receiverId : trade.senderId;
    this.gameGateway.server.to(notifyId).emit('notification', {
      type: 'error',
      title: 'Обмін скасовано',
      message:
        trade.senderId === userId
          ? 'Відправник скасував пропозицію обміну.'
          : 'Одержувач відхилив вашу пропозицію обміну.',
      duration: 5000,
    });
